...
Tick NVMeInterface::writeConfig(PacketPtr pkt) {
  ...
  if (offset < PCI_DEVICE_SPECIFIC) {
    ...
    if (offset == PCI0_BASE_ADDR0 || offset == PCI0_BASE_ADDR1) {
      registerTableBaseAddress = BARs[0]->addr() | ((uint64_t)BARs[1]->addr() << 32);
      registerTableSize = BARs[0]->size();
    }
    else if (offset == PCI0_BASE_ADDR4) {
      tableBaseAddress = BARs[4]->addr();
      tableSize = BARs[4]->size();
    }
    else if (offset == PCI0_BASE_ADDR5) {
      pbaBaseAddress = BARs[5]->addr();
      pbaSize = BARs[5]->size();
    }
  }
  else {
...
void NVMeInterface::regStats() {
  ...
  if (list.size() > 0) {
    pStats = new statistics::Scalar[list.size()]();
